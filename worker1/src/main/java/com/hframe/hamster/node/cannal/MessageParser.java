...
import com.hframe.hamster.common.db.dialect.DbDialect;
import com.hframe.hamster.common.db.dialect.mysql.MysqlDialect;
import com.hframe.hamster.common.db.dialect.oracle.OracleDialect;
import com.hframe.hamster.node.cannal.bean.*;
 ...
 * 数据对象解析
 * 
 * @author ** 2012-10-25 下午02:31:06
 * @version 4.1.2
 */
public class MessageParser {

    public static class MessageParseConfig{
    ...
    private MessageParseConfig parseConfig;

    public MessageParser(MessageParseConfig parseConfig) {
    ...
    public List<EventData> parse(List<Entry> datas) {
        ...
        boolean enableLoopbackRemedy = true; //pipelineParameter.isEnableRemedy() && pipelineParameter.isHome()  && pipelineParameter.getRemedyAlgorithm().isLoopback();
        String  systemMarkTable =null;    //pipeline.getParameters().getSystemMarkTable() // 双向同步标记表
        Integer remedyDelayThresoldForMedia = 60; //pipeline.getParameters().getRemedyDelayThresoldForMedia();  // 针对回环补救，如果反查速度过快，容易查到旧版本的数据记录，导致中美不一致，所以设置一个阀值，低于这个阀值的延迟不进行反查
            ...
            for (Entry entry : datas) {
                switch (entry.getEntryType()) {
                    ...
                    case ROWDATA:
                        ...
                        /*
                        // 判断是否是回环表retl_mark
                        boolean isMarkTable = tableName.equalsIgnoreCase(systemMarkTable);
                        if (isMarkTable) {
                        ...
                        */
                        ...
                        boolean isCompatibleLoopback = false;
                            ...
                            for (Entry bufferEntry : transactionDataBuffer) {
                                ...
                                for (EventData eventData : parseDatas) {
                                    ...
                                    if (needLoopback) {// 针对需要回环同步的
                                        ...
                                        if (now - eventData.getExecuteTime() > 1000 * remedyDelayThresoldForMedia) {
...
//        }
    ...
    private EventData internParse( Entry entry, RowChange rowChange, RowData rowData) {
        ...
        eventData.setExecuteTime(entry.getHeader().getExecuteTime());
        eventData.setBinLogPostion(entry.getHeader().getLogfileName() + "|" + StringUtils.leftPad(String.valueOf(entry.getHeader().getLogfileOffset()), 10, '0'));
        EventType eventType = eventData.getEventType();
        TableInfoHolder tableHolder = null;

//        if (!StringUtils.equalsIgnoreCase(parseConfig.getSystemSchema(), eventData.getSchemaName())) {
//            boolean useTableTransform = parseConfig.getUseTableTransform();
//            Table table = null;
//            DataMedia dataMedia = ConfigHelper.findSourceDataMedia(pipeline,
...
//        }
        ...
        // 判断一下是否需要all columns
        boolean isRowMode = parseConfig.getSyncMode().isRow(); // 如果是rowMode模式，所有字段都需要标记为updated
        boolean needAllColumns = isRowMode /*|| checkNeedAllColumns(pipeline) 默认都需要*/;
        ...
        // 变更前的所有旧的值
        Map<String, EventColumn> oldColumns = new LinkedHashMap<String, EventColumn>();
        ...
        if (eventType.isInsert()) {
            ...
            // 获取变更前的主键.
            for (Column column : beforeColumns) {
                //modify by zqh 变更类需要将旧值一起传给任务节点，作为条件计算依据
                oldColumns.put(column.getName(), copyEventColumn(column, isRowMode, tableHolder));
            }
            // 获取变更前的主键.
            for (Column column : beforeColumns) {
                if (isKey(tableHolder, tableName, column)) {
        ...
        List<EventColumn> oldcolumns = new ArrayList<EventColumn>(oldColumns.values());
        ...
        if (!keyColumns.isEmpty()) {
            ...
            if (eventData.getEventType().isUpdate() && oldcolumns != null && oldcolumns.size() > 0) { // update类型，如果存在主键不同,则记录下old
                // keys为变更前的主键
                eventData.setOldColumns(oldcolumns);
            }
...
//            throw new RuntimeException("this rowdata has no pks , entry: " + entry.toString() + " and rowData: "
//                                      + rowData);
        }
...
//    }
    ...
    private boolean isKey(TableInfoHolder tableHolder, String tableName, Column column) {
        ...
        if (dbColumn == null) {
            ...
            dbColumn = tableHolder.getTable().findColumn(column.getName(), false);
            if (dbColumn == null) {
                throw new RuntimeException(String.format("not found column[%s] in table[%s]",
                        column.getName(),
