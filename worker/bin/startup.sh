...
if [ -n "$str" ]; then
	...
	JAVA_OPTS="-server -Xms2048m -Xmx4096m -XX:NewSize=256m -XX:MaxNewSize=256m -XX:MaxPermSize=128m "
fi

JAVA_OPTS=" $JAVA_OPTS -Djava.awt.headless=true -Djava.net.preferIPv4Stack=true -Dfile.encoding=UTF-8"
HAMSTER_OPTS="-DappName=hamster-worker -Ddubbo.application.logger=slf4j -Dlogback.configurationFile=$logback_configurationFile -DnodeId=$(cat $hamsterNodeIdFile)"
...
then
  ...
  cd $bin_abs_path
	...
	$JAVA $JAVA_OPTS $JAVA_DEBUG_OPT $HAMSTER_OPTS -classpath .:$CLASSPATH com.hframework.datacenter.hamster.worker.HamsterWorker 1>>$base/logs/node/node.log 2>&1 &
