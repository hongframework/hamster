...
/**
  ...
  */
class MysqlScanExtractor(prototypeKey: String, deployMeta: DeployJobNodeMeta, var __jobNodeMeta: JobNodeMeta)
  extends AbstractExtractor(prototypeKey, deployMeta, __jobNodeMeta, __jobNodeMeta.jobMeta){

  type ParseResult = (Int, String, List[String], List[String])

  @volatile var (operType, filterExpression, sourceColumns, dataFilterVars): ParseResult = parse
  ...
  override def reload(newJobNodeMeta: JobNodeMeta): Unit = {
    ...
    val result = parse
    operType = result._1
    filterExpression = result._2
    sourceColumns = result._3
    dataFilterVars = result._4
  }

  def parse(): ParseResult = {
    val operType = {
      val tempType = jobNode.getInt(MysqlScan.operType, must = false)
      if (tempType == 0) 1
      else tempType
    }
    val filterExpression = jobNode.getString(MysqlScan.filterExpression)
    val sourceColumns = jobNode.getStrings(MysqlScan.sourceColumns)
    ...
    (operType, filterExpression, sourceColumns, dataFilterVars)
  }
