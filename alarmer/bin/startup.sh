...
HAMSTER_OPTS="-DappName=hamster-alarmer -Ddubbo.application.logger=slf4j -Dlogback.configurationFile=$logback_configurationFile -DnodeId=$(cat $hamsterNodeIdFile)"
...
then
  ...
  cd $bin_abs_path
	...
	$JAVA $JAVA_OPTS $JAVA_DEBUG_OPT $HAMSTER_OPTS -classpath .:$CLASSPATH com.hframework.datacenter.hamster.alarmer.HamsterAlarmer 1>>$base/logs/node/node.log 2>&1 &
